...
export async function Parser(ctx: Interpreter, input: string): Promise<SendData> {
    ...
    const attachments: Array<Discord.AttachmentBuilder | null> = [];
 ...
 * }
 */
export function EmbedParser(_ctx: Interpreter, content: string): Discord.EmbedBuilder {
 ...
 */
export async function ActionRowParser(ctx: Interpreter, content: string): Promise<Discord.ActionRowBuilder | null> {
...
): Discord.AttachmentBuilder | null {
    if (type === 'attachment') {
        const [name = 'attachment.png', url] = splitEscaped(rawContent);
        if (!url) return null;
        return new Discord.AttachmentBuilder(url, { name });
    }
            ...
            case 'actionrow': {
                ...
                if (row)
                    container.components.push(
                        row.toJSON() as Discord.APIActionRowComponent<Discord.APIComponentInMessageActionRow>
                    );
                continue;
            }
