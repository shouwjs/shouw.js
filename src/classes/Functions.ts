...
import { type Objects, type ParamType, type Interpreter, type ShouwClient, Functions } from '../index.js';
 ...
 * functions.new(<FunctionData>) // Create a new function
 */
export class FunctionsManager extends Collective<string, Functions> {
     ...
     */
    public new(data: FunctionData): FunctionsManager {
            ...
            func.code =
                data.code !== ''
                    ? async (ctx: Interpreter) => {
                          const result = await new ctx.interpreter(
                              {
                                  ...func,
                                  code: data.code ?? ''
                              },
