    ...
    if (interaction.isMessageComponent()) {
        ...
        for (const command of commands) {
            if (command.name?.includes('$') && command.name !== '$') {
                command.name = await INIT({ code: command.name }, interaction, [interaction.customId], client);
            }
         ...
         * @type {CommandData[]}
         */
        const commands = client.commands.interactionCreate.modal.filter((cmd: CommandData) =>
            cmd.name
                ? Array.isArray(cmd.name)
                    ? cmd.name?.includes(interaction.customId)
                    : cmd.name === interaction.customId
         ...
         * @type {CommandData[]}
         */
        const commands = client.commands.interactionCreate.slash.filter(
